(function() {
	...
	Ext.define('Ext.ux.Router', {
		...
		init: function(app) {
			...
			me.addEvents(
				/**
				 * @event routemissed Fires when no route is found for a given URI Token
				 * @param {String} uri The URI Token
				 */
				'routemissed',

				/**
				 * @event beforedispatch Fires before loading controller and calling its action. Handlers can return false to cancel the
				 *        dispatch process.
				...
				/**
				 * @event dispatch Fires after loading controller and calling its action.
				 * @param {String} uri URI Token.
				 * @param {Object} match Route that matched the URI Token.
				 * @param {Object} params The params appended to the URI Token.
		...
		routeMatcher: function(app, route, rules) {
			...
			if (!routeObj.controller && Ext.isDefined(Ext.global.console)) {
				Ext.global.console.error("[Ext.ux.Router] Config 'controller' can't be undefined", route,
						rules);
			}

			if (!routeObj.action && Ext.isDefined(Ext.global.console)) {
				Ext.global.console
						.error("[Ext.ux.Router] Config 'action' can't be undefined", route, rules);
			}
			// </debug>
		...
		/**
		 * Receives a url token and goes trough each of of the defined route objects searching for a match.
		 *
		 * @private
		 */
