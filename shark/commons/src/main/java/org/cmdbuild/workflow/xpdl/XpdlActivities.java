...
abstract class XpdlActivities {
	...
	XpdlActivities(final XpdlDocument doc) {
		this.doc = doc;
	}
	...
	public final XpdlActivity createActivity(final String activityId) {
		doc.turnReadWrite();
		final Activity a = (Activity) activities().generateNewElement();
		a.setId(activityId);
		activities().add(a);
	...
	public final List<XpdlActivity> getStartActivities() {
		doc.turnReadOnly();
		final List<XpdlActivity> startingActivities = new ArrayList<XpdlActivity>();
	...
	public final List<XpdlActivity> getManualStartActivitiesRecursive() {
		...
		for (final XpdlActivity sa : getStartActivities()) {
			if (sa.isManualType()) {
				out.add(sa);
