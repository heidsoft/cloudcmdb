...
public class StartWorkflowWidgetFactory extends ValuePairWidgetFactory {
	...
	protected Widget createWidget(final WidgetDefinition definition) {
		final String filterType = readString(definition.get(WORKFLOW_FILTER_TYPE));
		if (filterType != null && CQLFILTERTYPE.equals(CQLFILTERTYPE)) {
			 ...
			 */
			final String filter = readString(definition.get(WORKFLOW_FILTER));
			...
			final String workflowCode = readString(definition.get(WORKFLOW_CODE));
