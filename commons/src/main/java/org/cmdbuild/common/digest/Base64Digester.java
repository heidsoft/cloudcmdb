...
public class Base64Digester implements Digester {
	...
	@Override
	public String decrypt(final String encodedBase64Password) {
		try {
			final byte[] encryptedPasswordBytes = new sun.misc.BASE64Decoder().decodeBuffer(encodedBase64Password);
			final byte[] passwordBytesAsUTF8Encoding = getCiphers().decrypt(encryptedPasswordBytes);
